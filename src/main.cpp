// =====================================================
// IoT Platform - ESP32-S3 Gateway v5.0 (GATEWAY GENÉRICO)
// =====================================================
// HISTÓRICO DE CORREÇÕES (herdado da v4.1):
...
//     bloqueantes de connectWiFi()/connectMQTT()
//   • [FIX] WiFi.setSleep(false) — NÃO USAR com BLE ativo: quebra a
//     coexistência WiFi+BT do S3 e causa abort em coex_core_enable
//     (ver PENDÊNCIAS). Mantido comentado aqui só como registro.
//   • [FIX] AP de configuração agora tem senha
//   • [NOVO] Scan WiFi e BLE portados do v3.0
// v5.0 — MUDANÇA DE ARQUITETURA (documentar na defesa do TCC):
//   • [REMOVIDO] Todo o caminho local/EZ-mode da Tuya saiu do
//     firmware: tuya_devices.h, tuya_v35.h, tuya_dispatch.h,
//     tuya_discovery.h, pairing_provider.h. Esses arquivos
//     implementavam o protocolo local v3.5 + descoberta por UDP
//     broadcast + pareamento via AP-mode (engenharia reversa),
//     abandonados em favor da API Cloud oficial da Tuya.
//   • [REMOVIDO] tuya_cloud_client.h e secrets.h (client_id/
//     client_secret da Tuya) também saíram da ESP32. Essa
//     credencial é do projeto Cloud inteiro, compartilhada por
//     todos os usuários do sistema — não deve viver numa ESP32
//     fisicamente instalada na casa de terceiros. Essa lógica
//     passa a viver inteiramente no backend/site, que fala HTTPS
//     com a Tuya Cloud API e manda comandos já traduzidos por
//     MQTT pro gateway. A ESP32 agora é agnóstica de ecossistema.
//   • [NOVO] Gateway passa a focar em: MQTT robusto, scan
//     WiFi/BLE com alcance melhorado, e bonding BLE persistente
//     (NimBLE Security Manager) para pareamento com dispositivos
//     BLE genéricos — este último ainda em desenvolvimento.
// =====================================================
// PENDÊNCIAS CONHECIDAS (documentando para a defesa do TCC):
...
//   • Controle de dispositivos Tuya passou a ser responsabilidade
//     exclusiva do backend (fala direto com a Tuya Cloud API);
//     a ESP32 não despacha mais comando Tuya localmente.
//   • Bonding BLE (pareamento com dispositivo genérico via
//     Security Manager) ainda não está ligado ao fluxo de
//     comando/controle — próximo passo do TCC.
// =====================================================
...
// [NOVO] Potência máxima de TX do rádio BLE — varia por chip. O S3 (o
// que esse gateway usa) e o C3/C2 aguentam até +21dBm; H2/C6 até +20dBm;
// os demais (ex: ESP32 "clássico") ficam no teto mais conservador de +9dBm.
...
#else
#define MAX_TX_POWER ESP_PWR_LVL_P9   // Default (ex: ESP32 clássico)
#endif
...
void publishBLEScan();
void processPairing(const String& msg);
void pairingProviderPoll();
...
void printConfigBanner() {
    Serial.println("\n╔═══════════════════════════════════════════════════╗");
    Serial.println("║         IoT Gateway ESP32-S3  v5.0               ║");
    Serial.println("╠═══════════════════════════════════════════════════╣");
...
bool connectWiFi() {
    ...
    // [NOVO v5.0] Potência máxima de TX do rádio WiFi (19.5 dBm, teto
    // permitido pelo ESP32-S3). Isso melhora o alcance tanto da conexão
    // STA quanto do scan ativo (as probe requests do scan são
    // transmitidas pela ESP32 — quanto mais forte, mais longe uma AP
    // consegue "ouvir" e responder, ampliando o raio útil de detecção).
    WiFi.setTxPower(WIFI_POWER_19_5dBm);
    // [CORRIGIDO] setSleep(false) foi removido daqui — com BLE também
    // ativo (coexistência WiFi+BT no S3), o modem sleep do WiFi PRECISA
    // ficar habilitado. Desligar causava abort em coex_core_enable:
    // "Error! Should enable WiFi modem sleep when both WiFi and
    // Bluetooth are enabled!!!!!!". Se um dia o BLE for desativado por
    // completo, essa chamada pode voltar com segurança.
...
#include "ble_pairing_provider.h" 
...
// =====================================================
// PAREAMENTO (placeholder v5.0)
// =====================================================
// [REMOVIDO] O antigo fluxo de pareamento AP-mode da Tuya
// (pairing_provider.h + provisioning_ap_mode.h) saiu daqui — era
// especifico do EZ-mode/AP-mode que abandonamos.
// TODO: implementar aqui o fluxo de bonding BLE genérico:
//   scan -> connect (ble_gap_connect) -> pareamento via Security
//   Manager (callbacks de passkey/confirmação) -> salvar bond na NVS
//   -> reportar sucesso/erro por TOPIC_PAIRING.
// Por ora só confirma recebimento pro dashboard não travar esperando
...
inline void processPairing(const String& msg) {
    ...
    if (strcmp(action, "pair") == 0) {
        ...
        if (mac[0] == '\0') {
            resp["ok"] = false;
            resp["step"] = "validation";
            resp["message"] = "Campo 'mac' obrigatorio";
        } else {
            BlePairing::PairResult r = BlePairing::pair(mac);
            resp["ok"]      = r.ok;
            resp["step"]    = r.step;
            resp["mac"]     = mac;
        ...
        resp["ok"] = false;
        resp["step"] = "unknown_action";
        resp["message"] = String("Acao desconhecida: ") + action;
    }

    mqttPublishDoc(TOPIC_PAIRING.c_str(), resp);
}

inline void pairingProviderPoll() {
    // noop até o bonding BLE ser implementado
}
...
void processCommand(const String& msg) {
    ...
    // [REMOVIDO v5.0] "register_device" (id/ip/local_key) era exclusivo
    // do protocolo local Tuya v3.5. Registro de dispositivo Tuya agora
    // é responsabilidade do backend (fala direto com a Tuya Cloud API).
    // TODO: reintroduzir "register_device" para dispositivos BLE
    // pareados (bonding) assim que o fluxo de bonding estiver ligado
    // aqui — provavelmente recebendo { "mac": "...", "nome": "..." }.
    else {
...
void processControl(const String& msg) {
    ...
    // [MUDANÇA v5.0] Controle de dispositivos Tuya não passa mais pela
    // ESP32 — o backend fala direto com a Tuya Cloud API. Este tópico
    // (TOPIC_CONTROL) fica reservado para dispositivos pareados
    // localmente via BLE (bonding), que ainda não está ligado ao
    // dispatch de comando. Por enquanto, só confirma recebimento e
    // reporta "não implementado" em vez de falhar silenciosamente.
    // TODO: quando o bonding BLE estiver pronto, trocar por algo como
    //   bool ok = sendBleCommand(deviceId, turnOn);
    bool ok = false;
    const char* naoImplementado = "Controle local (BLE) ainda nao implementado nesta build";
    ...
    response["success"] = ok;
    response["message"] = ok ? "Comando executado" : naoImplementado;
    response["timestamp"] = getTimestamp();
    mqttPublishDoc(TOPIC_DEVICE_RESPONSE.c_str(), response);

    sendResponse("control_ack", cmdId, ok, ok ? "Comando processado" : naoImplementado);
}
...
void setup() {
    ...
    Serial.println("\n\n╔════════════════════════════════════════════════╗");
    Serial.println("║   IoT Platform - ESP32-S3 Gateway v5.0        ║");
    Serial.println("║   Gateway Genérico (MQTT + BLE Bonding)       ║");
    Serial.println("╚════════════════════════════════════════════════╝\n");
    ...
    // =====================================================
    NimBLEDevice::init(deviceHostname);

    // [NOVO v5.0] Potência máxima de TX do rádio BLE — usa o teto real
    // do chip-alvo (ver macro MAX_TX_POWER acima). Não melhora a
    // sensibilidade de recepção do scan passivo em si, mas aumenta
    // bastante o alcance de conexão/pareamento (GAP connect), que é
    // transmitido pela ESP32 — relevante pro bonding com dispositivo
    // distante.
    NimBLEDevice::setPower(MAX_TX_POWER);

    // [NOVO v5.0] Bonding BLE persistente (Security Manager).
    // Equivalente, na API do NimBLE-Arduino, aos campos ble_hs_cfg
    // que se configuraria manualmente no ESP-IDF puro:
    //   ble_hs_cfg.sm_bonding = 1;
    //   ble_hs_cfg.sm_sc      = 1;
    //   ble_hs_cfg.sm_mitm    = 0;
    //   ble_hs_cfg.sm_our_key_dist   = BLE_SM_PAIR_KEY_DIST_ENC | BLE_SM_PAIR_KEY_DIST_ID;
    //   ble_hs_cfg.sm_their_key_dist = BLE_SM_PAIR_KEY_DIST_ENC | BLE_SM_PAIR_KEY_DIST_ID;
    //   ble_store_config_init(); // persistência em NVS
    // O NimBLE-Arduino já chama ble_store_config_init() internamente
    // dentro de NimBLEDevice::init(), então a persistência em NVS dos
    // bonds já fica ativa sem precisar chamar isso manualmente — só
    // precisa garantir CONFIG_BT_NIMBLE_NVS_PERSIST=y no sdkconfig
    // (ou, se estiver via Arduino IDE puro, isso já vem habilitado
    // por padrão no core NimBLE-Arduino).
    NimBLEDevice::setSecurityAuth(/*bonding=*/true, /*mitm=*/false, /*secureConnections=*/true);
    // BLE_HS_IO_NO_INPUT_OUTPUT = pareamento "Just Works" (sem passkey).
    // TODO: trocar dinamicamente conforme a IO capability do periférico
    // (ex: BLE_HS_IO_DISPLAY_ONLY se o dispositivo pedir confirmação
    // de passkey na tela dele) quando o fluxo de bonding for implementado.
    NimBLEDevice::setSecurityIOCap(BLE_HS_IO_NO_INPUT_OUTPUT);

    bleScan = NimBLEDevice::getScan();
    bleScan->setAdvertisedDeviceCallbacks(&bleCB, false);
    // [NOVO v5.0] Scan ativo: solicita o scan response do dispositivo
    // (mais dados pra classificação: nome completo, UUIDs) e, por
    // exigir um pacote extra de ida-e-volta, também empurra um pouco
    // o alcance útil de detecção comparado ao scan passivo anterior.
    bleScan->setActiveScan(true);
    bleScan->setInterval(160);   // ~100ms (160 * 0.625ms) — mais denso
    bleScan->setWindow(150);     // janela quase igual ao intervalo = radio quase sempre ouvindo
    Serial.println("[BLE] ✅ NimBLE inicializado (bonding habilitado, TX no maximo do chip)");
